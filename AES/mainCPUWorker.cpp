/*
 ...
 Description   : Differs from main.cpp only in the worker part, where the CPU implementations are called.
 To build use  : make
 ============================================================================
 */
  ...
  if (rank == 0)
    {
      ...
      assert (iobuf != NULL);
      int retfread = fread (iobuf, 1, lSize, f);
      fclose (f);
  ...
  else                          // CPU worker
    {
      ...
      int pos = -1;
      int totalWork=0;
      workItemSize = (workItemSize / 16 + 1) * 16;      // making sure enough space for zero-padding is available
      ...
      // get 1st assigned block location
      ...
      while (pos >= 0)
        {
          ...
          // Padding is added at the end of the array, to produce an integer
          // number of 16-byte blocks
          int paddedSize = ceil (actualSize / 16.0) * 16;  
          for (int k = actualSize; k < paddedSize; k++)
            iobuf[k] = ' ';

          // Assigned data block is processed in 16-byte blocks at a time
          int dataPos;
          for (dataPos = 0; dataPos < actualSize; dataPos += 16)
            {
              // encrypt 16-byte block
              rijndaelCPUEncrypt (rk, nrounds, iobuf + dataPos, iobuf + dataPos);
            }

          // Return results, and their location in the overall ciphertext
          MPI_Send (&pos, 1, MPI_INT, 0, TAG_RES, MPI_COMM_WORLD);
